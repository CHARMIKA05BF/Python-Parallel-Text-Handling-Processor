...
def save_to_csv(rows: List[Dict[str, Any]], out_path: str) -> str:
    if not rows:
        ...
        return out_path
    # Prefer pandas if available for convenience; otherwise fallback to csv module
    if pd is not None:
        df = pd.DataFrame([{
            "id": r.get("id"),
            "uid": r.get("uid"),
        ...
        logger.info("Saved %d rows to %s (via pandas)", len(rows), out_path)
        return out_path

    # Fallback using csv.writer
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        fieldnames = ["id", "uid", "score", "details", "ts", "text"]
    ...
    logger.info("Saved %d rows to %s (via csv)", len(rows), out_path)
    return out_path
